...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleMouseDownAndEnter(ev, actualThis, mouseEvent) {
        ...
        if (gridBoard.algoIsRunning === true) {
            return;
        }
    ...
    function addNodeEventListeners() {
        /* Add the event listener after the infobox has been closed so that the user
        can't modify the board before. Every individual nodes needs its own set of
        event listener because the 'mouseenter' event doesn't work when adding an
        event lister to the board and using event.target */
        let nodes = [];
        
        for (let row = 0; row < gridBoard.rows; row++) {
            for (let col = 0; col < gridBoard.columns; col++) {
                nodes.push(document.getElementById(`node-${row}-${col}`));
            }
        }

        for (let i = 0; i < nodes.length; i++) {
            nodes[i].addEventListener('mousedown', function(ev) {
                handleMouseDownAndEnter(ev, this, 'mouseDown');
            });
            ...
            nodes[i].addEventListener('mouseup', function(ev) {
                ev.preventDefault();
